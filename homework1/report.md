...
- **與作業要求關聯**：這部分為後續測試奠定基礎，支援作業要求的測試資料量（如 n = 500, 1000, ..., 5000）。
...
- **與作業要求關聯**：這些結構支援排序演算法的實現，模擬作業中提到的「n 個整數」排序需求。
...
- **與作業要求關聯**：實現了插入排序，並支援最壞情況測試（後續資料生成會使用遞減序列，如 n, n-1, ..., 1）。
...
- **功能**：
  ...
  - `QuickSort`：啟動排序並計時。
- **注意**：作業要求使用「median-of-three」方法選擇樞軸，但此實現僅使用左端元素，需修改以符合要求。
- **與作業要求關聯**：實現快速排序，並準備使用隨機排列測試最壞情況。
...
- **功能**：
  ...
  - `MergeCut`：遞迴分割陣列。
  - `MergeSort`：啟動排序並計時。
- **注意**：作業要求使用迭代方法，但此實現為遞迴版本，需修改以符合要求。
- **與作業要求關聯**：實現合併排序，並使用遞減序列模擬最壞情況。
...
- **與作業要求關聯**：
  - 支援最壞情況測試：插入排序使用遞減序列，堆積排序使用隨機排列。
  - 記錄時間並輸出到文件，符合報告要求。

---

### 8. 與作業要求的差距
- **快速排序**：未實現 median-of-three，需修改。
- **合併排序**：使用遞迴而非迭代版本，需調整。
- **測試資料量**：目前固定為 5000，未涵蓋作業要求的 500 到 5000 範圍，需修改 `CASE_ITEMS` 並多次執行。
- **複合排序函數**：尚未實現，需根據測試結果設計。
